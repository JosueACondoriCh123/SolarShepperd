import { Activity, Bell, CircleDotDashed, CloudSun, Map, Navigation, Sprout } from "lucide-react";
...
export function DashboardPage() {
  ...
  return <div className="page">
    ...
    <div className="dashboard-grid">
      ...
      <section className="panel quick-actions"><span className="panel-kicker">QUICK ACCESS</span><h2>Move from signal to evidence</h2><div><Link to={pathFor("field-flow")}><CircleDotDashed />Field decision loop</Link><Link to={pathFor("telemetry")}><Activity />Check station</Link><Link to={pathFor("landscape")}><Map />Inspect landscape</Link><Link to={pathFor("routes")}><Navigation />Plan route</Link><Link to={pathFor("samples")}><Sprout />Capture sample</Link></div></section>
    </div>
